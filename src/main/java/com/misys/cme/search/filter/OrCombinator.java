package com.misys.cme.search.filter;

import java.util.ArrayList;
import java.util.Collections;
import java.util.List;

import com.misys.cmeobject.search.Queries.Combinator;
import com.misys.cmeobject.search.Queries.CombinatorNode;
import com.misys.cmeobject.search.Queries.Query;

/**
 * Represents the boolean OR operator in search filters.
 * This is specified by a "$or" field in the filter JSON.
 */
public class OrCombinator implements CombinatorNode {
	private List<Query> filters;
	
	/**
	 * Constructs a OrCombinator from the given List of Query.
	 * @param filters List of Query to be ORed together.
	 */
	public OrCombinator(List<? extends Query> filters) {
		this.filters = new ArrayList<>();
		this.filters.addAll(filters);
	}
	
	/**
	 * Gets the List of Query that will be ORed together by this.
	 * @return List of Query this OrCombinator is combining.
	 * @note The returned list is unmodifiable.
	 */
	@Override
	public List<Query> getChildren() {
		return Collections.unmodifiableList(filters);
	}
	
	/**
	 * Creates and returns a string representation of this.
	 * @return A String representing this object.
	 */
	@Override
	public String toString() {
		String output = "OrCombinator{filters=[";
		boolean haveAddedFilter = false;
		for (Query filter : filters) {
			if (!haveAddedFilter) {
				output += filter.toString();
				haveAddedFilter = true;
			} else {
				output += ", " + filter.toString();
			}
		}
		output += "]}";
		return output;
	}

	/**
	 * Gets the type of this Combinator.
	 * @return The type of this Combinator.
	 */
	@Override
	public Combinator getCombinatorType() {
		return Combinator.OR;
	}
}
